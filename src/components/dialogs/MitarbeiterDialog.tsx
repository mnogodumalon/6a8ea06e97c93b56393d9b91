...
export type MitarbeiterDialogDefaults = Omit<Mitarbeiter['fields'], 'beschaeftigungsart'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  beschaeftigungsart: LOOKUP_OPTIONS['mitarbeiter']?.['beschaeftigungsart'] ?? [],
};
...
export function MitarbeiterDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterDialogProps) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('mitarbeiter', 'telefon')}</Label>
        <Input
          id="telefon"
    ...
    'beschaeftigungsart': (
      <div key="beschaeftigungsart" className="space-y-1.5">
        <Label htmlFor="beschaeftigungsart">{fieldLabel('mitarbeiter', 'beschaeftigungsart')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('mitarbeiter', 'beschaeftigungsart', 'vollzeit') ?? 'Vollzeit'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitarbeiter', 'beschaeftigungsart', 'teilzeit') ?? 'Teilzeit'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitarbeiter', 'beschaeftigungsart', 'minijob') ?? 'Minijob'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitarbeiter', 'beschaeftigungsart', 'aushilfe') ?? 'Aushilfe'}
          </button>
        </div>
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "abteilung": "Abteilung", "position": "Position", "beschaeftigungsart": "Beschäftigungsart", "notizen": "Notizen"};
