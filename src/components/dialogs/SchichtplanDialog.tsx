/**
 * SchichtplanDialog — pre-generated create/edit dialog for Schichtplan.
 *
 * Props: open, onClose, onSubmit(fields) => Promise<void>, defaultValues?,
 * recordId? (pass when EDITING — enables the attachments section),
 * mitarbeiterList (full hook array — resolves the Mitarbeiter applookup),
 * schichtenList (full hook array — resolves the Schichten applookup),
 * enablePhotoScan?, enablePhotoLocation?.
 *
 * defaultValues is SHAPE-TOLERANT and its prop type is the EXPORTED
 * SchichtplanDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Schichtplan['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<SchichtplanDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Schichtplan';
...
/** Widened prefill type for SchichtplanDialog.defaultValues — see file header. */
export type SchichtplanDialogDefaults = Schichtplan['fields'];

interface SchichtplanDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Schichtplan['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: SchichtplanDialogDefaults;
...
export function SchichtplanDialog({ open, onClose, onSubmit, defaultValues, recordId, mitarbeiterList, schichtenList, enablePhotoScan = true, enablePhotoLocation = true }: SchichtplanDialogProps) {
  const [fields, setFields] = useState<Partial<Schichtplan['fields']>>({});
  ...
  // Inline-Create state for "Schichten" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraSchichten` list, and select it in
  // the originating Combobox via the captured `createSchichtenField`.
  const [createSchichtenOpen, setCreateSchichtenOpen] = useState(false);
  const [createSchichtenInitial, setCreateSchichtenInitial] = useState('');
  const [createSchichtenField, setCreateSchichtenField] = useState<string>('');
  const [extraSchichten, setExtraSchichten] = useState< Schichten[]>([]);
  const schichtenListAll = useMemo(
    () => [...schichtenList, ...extraSchichten],
    [schichtenList, extraSchichten],
  );
  function openCreateSchichten(fieldKey: string, q: string) {
    setCreateSchichtenField(fieldKey);
    setCreateSchichtenInitial(q);
    setCreateSchichtenOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['datum', 'mitarbeiter', 'schicht'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Schichtplan['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'schichtplan');
      await onSubmit(clean as Schichtplan['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="schicht" entity="Schichten">\n${JSON.stringify(schichtenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitarbeiter", "schicht"]);
        ...
        const schichtName = raw['schicht'] as string | null;
        if (schichtName) {
          const schichtMatch = schichtenList.find(r => matchName(schichtName!, [String(r.fields.schicht_name ?? '')]));
          if (schichtMatch) merged['schicht'] = createRecordUrl(APP_IDS.SCHICHTEN, schichtMatch.record_id);
        }
        return merged as Partial<Schichtplan['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('schichtplan') })
    : t('new_entity', { entity: appLabel('schichtplan') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'schicht': (
      <div key="schicht" className="space-y-1.5">
        ...
        <Combobox
          ...
          value={extractRecordId(fields.schicht)}
          onChange={id => setFields(f => ({ ...f, schicht: id ? createRecordUrl(APP_IDS.SCHICHTEN, id) : undefined }))}
          onCreateNew={(q) => openCreateSchichten("schicht", q)}
          createLabel={t('create_in', { entity: appLabel('schichten') })}
        />
        {showErrors && !fields.schicht && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'arbeitsbereich': (
      <div key="arbeitsbereich" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"datum": "Datum", "mitarbeiter": "Mitarbeiter", "schicht": "Schicht", "arbeitsbereich": "Arbeitsbereich", "anmerkungen": "Anmerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitarbeiter": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "abteilung": "Abteilung", "position": "Position", "beschaeftigungsart": "Beschäftigungsart", "notizen": "Notizen"}, "schicht": {"schicht_name": "Schichtname", "kuerzel": "Kürzel", "startzeit": "Startzeit", "endzeit": "Endzeit", "schicht_beschreibung": "Beschreibung", "schicht_typ": "Schichttyp"}};
  ...
  return (
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SCHICHTPLAN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createSchichtenOpen && (
      <SchichtenDialog
        open={createSchichtenOpen}
        onClose={() => setCreateSchichtenOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createSchichtenEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Schichten;
            setExtraSchichten(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.SCHICHTEN, result.id);
            setFields(prev => ({ ...prev, [createSchichtenField]: url } as any));
          }
          setCreateSchichtenOpen(false);
        }}
        defaultValues={createSchichtenInitial
          ? ({ schicht_name: createSchichtenInitial } as any)
          : undefined}
      />
    )}
    </>
