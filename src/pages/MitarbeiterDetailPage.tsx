...
import { formEnhancements } from '@/config/form-enhancements/Mitarbeiter';
...
export default function MitarbeiterDetailPage() {
  ...
  const [record, setRecord] = useState<Mitarbeiter | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const list = await LivingAppsService.getMitarbeiter();
  ...
  async function handleDelete() {
    if (!record) return;
    await LivingAppsService.deleteMitarbeiterEntry(record.record_id);
    setDeleteOpen(false);
  ...
  if (!record) {
    return (
      <RecordViewEmpty
        title={t('not_found')}
        action={
          <Button variant="ghost" onClick={() => navigate('/mitarbeiter')}>
  ...
  return (
    <RecordView
      onBack={() => navigate('/mitarbeiter')}
      onEdit={() => setEditing(true)}
      backLabel={t('back')}
      editLabel={t('edit_button')}
    >
      <RecordHeader title={record.fields.vorname ?? appLabel('mitarbeiter')} />
