...
import { SchichtenViewDialog } from '@/components/dialogs/SchichtenViewDialog';
import { SchichtplanDialog } from '@/components/dialogs/SchichtplanDialog';
import { SchichtplanViewDialog } from '@/components/dialogs/SchichtplanViewDialog';
...
const MITARBEITER_FIELDS = [
  ...
  { key: 'beschaeftigungsart', label: 'Beschäftigungsart', type: 'lookup/select', options: [{ key: 'vollzeit', label: 'Vollzeit' }, { key: 'teilzeit', label: 'Teilzeit' }, { key: 'minijob', label: 'Minijob' }, { key: 'aushilfe', label: 'Aushilfe' }] },
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
];
const SCHICHTEN_FIELDS = [
  { key: 'schicht_name', label: 'Schichtname', type: 'string/text' },
  ...
  { key: 'schicht_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'schicht_typ', label: 'Schichttyp', type: 'lookup/select', options: [{ key: 'fruehschicht', label: 'Frühschicht' }, { key: 'spaetschicht', label: 'Spätschicht' }, { key: 'nachtschicht', label: 'Nachtschicht' }, { key: 'tagschicht', label: 'Tagschicht' }, { key: 'sonderschicht', label: 'Sonderschicht' }] },
];
const SCHICHTPLAN_FIELDS = [
  { key: 'datum', label: 'Datum', type: 'date/date' },
  { key: 'mitarbeiter', label: 'Mitarbeiter', type: 'applookup/select', targetEntity: 'mitarbeiter', targetAppId: 'MITARBEITER', displayField: 'vorname' },
  { key: 'schicht', label: 'Schicht', type: 'applookup/select', targetEntity: 'schichten', targetAppId: 'SCHICHTEN', displayField: 'schicht_name' },
  { key: 'arbeitsbereich', label: 'Arbeitsbereich', type: 'string/text' },
  { key: 'anmerkungen', label: 'Anmerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'mitarbeiter', pascal: 'Mitarbeiter' },
  { key: 'schichten', pascal: 'Schichten' },
  { key: 'schichtplan', pascal: 'Schichtplan' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichten': return (data as any).schichten as Schichten[] ?? [];
      case 'schichtplan': return (data as any).schichtplan as Schichtplan[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'schichtplan':
        ...
        lists.schichtenList = (data as any).schichten ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'schichtplan' && fieldKey === 'mitarbeiter') {
      const match = (lists.mitarbeiterList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'schichtplan' && fieldKey === 'schicht') {
      const match = (lists.schichtenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.schicht_name ?? '—';
    }
    return String(url);
  }, [getLookupLists]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    const raw: any[] = (() => {
      switch (entity) {
        case 'mitarbeiter': return MITARBEITER_FIELDS as any[];
        case 'schichten': return SCHICHTEN_FIELDS as any[];
        case 'schichtplan': return SCHICHTPLAN_FIELDS as any[];
        default: return [];
      }
    })();
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'schichten': return {
        create: (fields: any) => LivingAppsService.createSchichtenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtenEntry(id),
      };
      case 'schichtplan': return {
        create: (fields: any) => LivingAppsService.createSchichtplanEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateSchichtplanEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteSchichtplanEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'schichten' || dialogState?.entity === 'schichten') && (
        <SchichtenDialog
          open={createEntity === 'schichten' || dialogState?.entity === 'schichten'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichten' ? handleUpdate : (fields: any) => handleCreate('schichten', fields)}
          defaultValues={dialogState?.entity === 'schichten' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Schichten']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Schichten']}
        />
      )}
      {(createEntity === 'schichtplan' || dialogState?.entity === 'schichtplan') && (
        <SchichtplanDialog
          open={createEntity === 'schichtplan' || dialogState?.entity === 'schichtplan'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'schichtplan' ? handleUpdate : (fields: any) => handleCreate('schichtplan', fields)}
          defaultValues={dialogState?.entity === 'schichtplan' ? dialogState.record?.fields : undefined}
          mitarbeiterList={(data as any).mitarbeiter ?? []}
          schichtenList={(data as any).schichten ?? []}
      ...
      {viewState?.entity === 'schichten' && (
        <SchichtenViewDialog
          open={viewState?.entity === 'schichten'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichten', record: r }); }}
        />
      )}
      {viewState?.entity === 'schichtplan' && (
        <SchichtplanViewDialog
          open={viewState?.entity === 'schichtplan'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'schichtplan', record: r }); }}
          mitarbeiterList={(data as any).mitarbeiter ?? []}
          schichtenList={(data as any).schichten ?? []}
        />
      )}
