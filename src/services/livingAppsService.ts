...
import type { Mitarbeiter, Schichten, Schichtplan, CreateMitarbeiter, CreateSchichten, CreateSchichtplan } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- SCHICHTEN ---
  static async getSchichten(): Promise<Schichten[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Schichten[];
    return hydrateRecords(records, 'schichten');
  }
  static async getSchichtenEntry(id: string): Promise<Schichten | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Schichten;
    return hydrateRecords([record], 'schichten')[0];
  }
  static async createSchichtenEntry(fields: CreateSchichten): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SCHICHTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'schichten') });
    return { ...data, record_id: data.id };
  }
  static async updateSchichtenEntry(id: string, fields: Partial<CreateSchichten>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SCHICHTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichten') });
    return { ...data, record_id: data.id };
  }
  static async deleteSchichtenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SCHICHTEN}/records/${id}`);
  }

  // --- SCHICHTPLAN ---
  static async getSchichtplan(): Promise<Schichtplan[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTPLAN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Schichtplan[];
    return hydrateRecords(records, 'schichtplan');
  }
  static async getSchichtplanEntry(id: string): Promise<Schichtplan | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHICHTPLAN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Schichtplan;
    return hydrateRecords([record], 'schichtplan')[0];
  }
  static async createSchichtplanEntry(fields: CreateSchichtplan): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SCHICHTPLAN}/records`, { fields: cleanFieldsForApi(fields as any, 'schichtplan') });
    return { ...data, record_id: data.id };
  }
  static async updateSchichtplanEntry(id: string, fields: Partial<CreateSchichtplan>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SCHICHTPLAN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schichtplan') });
    return { ...data, record_id: data.id };
  }
  static async deleteSchichtplanEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SCHICHTPLAN}/records/${id}`);
  }
